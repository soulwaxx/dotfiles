...
3. Use a plain labeled 3D-ish placeholder (e.g. `application_server` or `dataServer`) with a clear label, or a generic device icon from [aws-icons-allied-telesis.md](aws-icons-allied-telesis.md) if it's hardware-shaped (servers, racks, network gear).
