...
> Vendored from [danyuchn/asd-ste100-skill](https://github.com/danyuchn/asd-ste100-skill) (MIT license, see `LICENSE`). Copied in statically so the skill stays available without cloning the upstream repo. The upstream `README.md` is intentionally omitted; its before/after table demonstrates a hedge-to-fact rewrite that this skill's own rules forbid — use `examples/before-after.md` for the corrected examples.
>
> Scope split for this dotfiles repo: this skill targets **machine-parsed strings** (tool/MCP descriptions, error messages, inter-agent instructions, system prompts). For **human-facing prose** (ADRs, PRs, commits, Slack, docs) use the `writing-style` skill instead. The two agree on active voice, one idea per sentence, and no marketing adjectives. They disagree on hedges — `writing-style` bans bare "may/might", this skill preserves them — because their readers differ. Route by reader.
