...
# Vendored from claude-obsidian v2.0.0 (claude_obsidian/lint_engine.py) with
# edits marked "OKF fork:", plus the v2.2.0 dot-directory walk prune backported
# below. Pure stdlib, performs zero writes. Upstream is
# https://github.com/AgriciDaniel/claude-obsidian (MIT, see LICENSE-claude-obsidian).
from json import loads as strict_json_loads

REPORT_VERSION = 1
ENGINE_VERSION = "1.1.1"
# OKF fork: OKF v0.2 makes `type` the only required field (§4.1), not
# upstream's six-field baseline. `last_updated` stays a recommended producer
# extension the vault still authors, but lint no longer flags its absence.
...
# OKF fork (backport of claude-obsidian v2.2.0): Obsidian hides every
# dot-prefixed folder from its own indexer (.obsidian, .vault-meta, and any
# user- or tool-created hidden directory such as a .raw ingest archive,
# .claude/.pi agent worktrees, or Obsidian's own .trash). The v2.0.0 vendor
# only skipped an explicit set, so a duplicate or archived page under a
# dot-prefixed folder still became a real link-resolution candidate — one
# healthy link then read as ambiguous_targets (and, on index pages,
# stale_index_entries). Prune all dot-prefixed directories, matching Obsidian,
# and keep only the two non-dot build dirs explicit. Add a name to the keeplist
# only for a confirmed, legitimate dot-prefixed vault-content directory that
# must still be walked; none is known today.
_IGNORED_WALK_DIRS = {
    "node_modules",
    "__pycache__",
}
_DOT_DIR_KEEPLIST: frozenset[str] = frozenset()
# OKF fork: files this vault generates rather than authors. OKF v0.2 reserves
# exactly index.md (directory listing) and log.md (update history); both carry
# no concept frontmatter and are linked by nothing, and log.md's
# `## <ts> | <paths>` backbone is headings with no body by design. Kept as one
# set so the categories below agree.
_OKF_GENERATED_NAMES = {"index.md", "log.md"}
...
class _Resolver:
    ...
    def resolve(self, link: _Link) -> list[_Target]:
        ...
        raw = raw.lstrip("/")
        # A trailing slash denotes a directory link. OKF represents a directory
        # by its generated index.md (the progressive-disclosure listing), so
        # `concepts/` resolves to `concepts/index.md` — the form sync.py emits
        # for every subdirectory. Without this, every generated directory link
...
def _transaction_reader() -> Any:
    """OKF fork: upstream's transaction core is not vendored. Plain read that
    still refuses to follow a symlinked path, which is the only property the
    lint pass depended on."""

    def read_vault_regular(root: Path, relative: str, limit: int = 0) -> bytes | None:
...
def _provenance_errors(root: Path, *, as_of: date) -> list[dict[str, str]]:
    """OKF fork: no-op. The source/claim ledgers were rejected for this vault;
    provenance stays instruction-level (references/research-discipline.md)."""

    return []


def _unused_provenance_errors(root: Path, *, as_of: date) -> list[dict[str, str]]:
...
def lint_vault(
    ...
    for group in basename_groups.values():
        ...
        if PurePosixPath(ordered[0].path).stem.casefold() == "_index":
            continue
        # OKF fork: every directory legitimately holds its own generated
        # index.md; that is the layout sync.py produces, not a collision.
        if f"{PurePosixPath(ordered[0].path).stem.casefold()}.md" in _OKF_GENERATED_NAMES:
            continue
    ...
    for page in wiki_pages:
        # OKF fork: generated files are exempt from page rules, exactly as
        # validate.py exempts them by name.
        if PurePosixPath(page.path).name.casefold() in _OKF_GENERATED_NAMES:
            continue
        missing = [
            field
