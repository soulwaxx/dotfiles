---
description: Bootstrap or check the Obsidian wiki vault. Reads the wiki skill and runs setup workflow.
---

Read the `wiki` skill. Then run the setup workflow:

1. Check if Obsidian is installed. If not, offer to install it (see `skills/wiki/references/plugins.md`). `obsidian-cli` is optional; filesystem tools are authoritative.
2. Check if this directory has a vault (look for `.obsidian/` folder). If yes, report current vault state.
3. Ask ONE question: "What is this vault for?"
